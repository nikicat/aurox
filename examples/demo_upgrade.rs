...
use pty_harness::{Pty, armed_after, dwell, has};
...
fn main() {
    ...
    // package's `pacman -U`. Neither half identifies it alone: the first
    // `Continue?` is still on screen, and the disclosed `-U` line prints
    // before dialoguer arms. Only a `Continue?` *after* that line does.
    pty.expect("aur install gate", |s| {
